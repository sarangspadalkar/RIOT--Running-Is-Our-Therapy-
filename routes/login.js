    ...
    async function (req, res) {
        if (req.session.theUser == null) {
            ...
            if (!login_errors.isEmpty()) {
                ...
                if (valid_user) {
                    var userdetails = await userDb.getUserPassword(
                        req.body.username
                    ); // Fetch a user details with username.
                    ...
                    if (userdetails) {
                        ...
                        if (req.body.password == userdetails.password) {
                            ...
                            var savedConnections =
                                await userConnectionDb.getUserConnectionList(
                            ...
                            var userProfile =
                                await userConnectionDb.saveUserProfile(
                                    req.session.theUser,
                                    savedConnections
                                );
                            if (userProfile) {
